...
const { loadCustomLexicon } = require('../services/custom-lexicon-service');

let lexiconData = null;
let customLexiconLoader = loadCustomLexicon;
...
function setCustomLexiconLoader(loader) {
  customLexiconLoader = typeof loader === 'function' ? loader : loadCustomLexicon;
}

function getCustomLexicon() {
  return customLexiconLoader ? customLexiconLoader() : { fillerWords: [], hedgeWords: [], vagueReplacements: {} };
}
...
function segmentText(text) {
  ...
  const customLexicon = getCustomLexicon();
  const fillerWords = [...new Set([...FILLER_WORDS, ...customLexicon.fillerWords])];
  const hedgeWords = [...new Set([...HEDGE_WORDS, ...customLexicon.hedgeWords])];
  const vagueToPrecise = { ...VAGUE_TO_PRECISE, ...customLexicon.vagueReplacements };
...
function analyzeText(text) {
  ...
  const vagueWords = [];
  words.forEach((word, idx) => {
    if (vagueToPrecise[word]) {
      vagueWords.push({
        word,
        position: idx,
        alternatives: vagueToPrecise[word]
      });
    }
  });
