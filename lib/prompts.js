...
const { getTrainingModePreset } = require('../config/training-modes');
...
function getRealtimePrompt(text, context, customPrompt) {
  ...
  const trainingMode = getTrainingModePreset(customPrompt?.trainingMode);

  // 拼接用户自定义规则
  let customBlock = `\n\n## 当前训练模式\n${trainingMode.label}：${trainingMode.realtimeFocus}`;
  if (customPrompt) {
...
报告开头第一句话固定为：「omelet 收到你的录音啦~~」（如果输入是逐字稿则改为「omelet 收到你的逐字稿啦~~」），然后空一行再开始正文。
...
数据:${stats.duration}秒 | ${stats.totalWords}字 | 填充词${stats.fillers}次 | 犹豫词${stats.hedges}次 | 笼统词${stats.vagueWords}次`
  ...
  let customBlock = `\n\n## 当前训练模式\n${trainingMode.label}：${trainingMode.reportFocus}`;
  if (customPrompt) {
    if (customPrompt.goals) {
      customBlock += `\n\n## 用户训练目标(报告中请重点关注这些方面)\n${customPrompt.goals}`;
    }
